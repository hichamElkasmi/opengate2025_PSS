...
public class SSRepo_CardsList implements SQLData {
 
	private final String REQUEST_TYPE = "CARDSLISTRQ";
	private final String RESPONSE_TYPE = "CARDSLISTRS";
	private final String PACKAGE_SQL = SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL = "getCardsList";
 
	@Getter @Setter
	private SSEnt_HeadInitClsRq request;
	@Getter @Setter
	private SSEnt_CardsListRs response;
 
	SS_LoggingImpl logr; 
	public SSRepo_CardsList() {
		response = new SSEnt_CardsListRs();
	...
	public SSRepo_CardsList(SSEnt_HeadInitClsRq request) {
		this();
		this.request = request;
	}
	
	 public SSEnt_CardsListRs getResponse() {
	...
	public void readSQL(SQLInput stream, String typeName) throws SQLException {
	      try{
	    	  logr.getLog().trace("start readSQL, SSRepo_CardsList");
	    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
	            if(ent_Status.isAuthorized()){
	            	logr.getLog().trace("Authorized readSQL, SSRepo_CardsList");
	        		Object[] objects = (Object[]) stream.readArray().getArray();
	        		List<SSEnt_CardInfo> cards = Arrays.stream(objects).map(o -> (SSEnt_CardInfo)o).collect(Collectors.toList());
	        		response.setCards(cards);
	            }
	            else
	            {
	            	logr.getLog().trace("Error readSQL, SSRepo_CardsList");
	                response.setStatus(new SSEnt_Status(getMsg("ST_REJECTED"), 
	                		ent_Status.getErrorCode(), ent_Status.getErrorMsg()));
	            }
	            logr.getLog().trace("end readSQL, SSRepo_CardsList");
	        }
	        catch(NullPointerException e){
	        	logr.getLog().trace("Pas de données disponible list des SSRepo_CardsList");
	    		response.setStatus(new SSEnt_Status(getMsg("ST_NODATA"), getMsg("ER_CLEAN")));
	        }
	        catch(Exception e){
		    	logr.getLog().error("readSQL, SSRepo_CardsList",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		logr.getLog().trace("start writeSQL, SSRepo_CardsList");
		stream.writeObject(request.getInitiator());
		logr.getLog().trace("start writeSQL, SSRepo_CardsList");
	}
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_CardsListRs processing(DataSource dataSource) {
		try {
			logr.getLog().trace("start processing, SSRepo_CardsList");
			...
			SimpleJdbcCall jdbcCall = new SimpleJdbcCall(dataSource)
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_CardsList.class, auxiliaryTypes)));
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_CardsList)jdbcCall.executeObject(SSRepo_CardsList.class,in)).getResponse();
			logr.getLog().trace("end processing, SSRepo_CardsList");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_CardsList",e);

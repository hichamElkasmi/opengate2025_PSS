...
public class SSRepo_ChannelsList implements SQLData {

	private final String REQUEST_TYPE = "CHANNELSLISTRQ";
	private final String RESPONSE_TYPE = "CHANNELSLISTRS";
	
	private final String PACKAGE_SQL = SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL = "GETCHANNELSLIST";

	@Getter @Setter
	private SSEnt_HeadInitCrdRq request;
	@Getter @Setter
	private SSEnt_ChannelsListRs response;

	SS_LoggingImpl logr; 
	
	public SSRepo_ChannelsList() {
		response = new SSEnt_ChannelsListRs();
		response.setStatus(new SSEnt_Status(getMsg("ST_APPROVED"), getMsg("ER_CLEAN")));
		logr = new SS_LoggingImpl();
		logr.setClazz(getClass());
	}
	
	public SSRepo_ChannelsList(SSEnt_HeadInitCrdRq request) {
		this();
	...
	public void readSQL(SQLInput stream, String typeName) throws SQLException {
      ...
      try{
    	  logr.getLog().trace("start readSQL, SSRepo_ChannelsList");
    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
            if(ent_Status.isAuthorized()){
            	 logr.getLog().trace("Authorized readSQL, SSRepo_ChannelsList");
            	Object[] objects = (Object[]) stream.readArray().getArray();
        		List<SSEnt_Channel> channels = Arrays.stream(objects).map(o -> (SSEnt_Channel)o).collect(Collectors.toList());
        		response.setChannels(channels);
            }
            else
            {
            	 logr.getLog().trace("Error readSQL, SSRepo_ChannelsList");
            ...
            logr.getLog().trace("end processing, SSRepo_ChannelsList");
    	  	...
    	  	logr.getLog().trace("Pas de données disponible list des SSRepo_ChannelsList");
  			response.setStatus(new SSEnt_Status(getMsg("ST_NODATA"), getMsg("ER_CLEAN")));
      }
      catch(Exception e){
	    	logr.getLog().error("readSQL, SSRepo_ChannelsList",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		 logr.getLog().trace("start writeSQL, SSRepo_ChannelsList");
		stream.writeObject(request.getInitiator());
		 logr.getLog().trace("end writeSQL, SSRepo_ChannelsList");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_ChannelsListRs processing(DataSource dataSource) {
		try {
			 logr.getLog().trace("start processing, SSRepo_ChannelsList");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_ChannelsList.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_ChannelsList)jdbcCall.executeObject(SSRepo_ChannelsList.class,in)).getResponse();
			 logr.getLog().trace("end processing, SSRepo_ChannelsList");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_ChannelsList",e);

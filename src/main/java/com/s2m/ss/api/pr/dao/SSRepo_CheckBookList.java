...
public class SSRepo_CheckBookList implements SQLData {

	private final String REQUEST_TYPE 	= "CHECKSIZESLISTRQ";
	private final String RESPONSE_TYPE 	= "CHECKSIZESLISTRS";
	
	private final String PACKAGE_SQL 	= SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL 	= "getCheckSizesList";

	@Getter @Setter
	private SSEnt_HeadInitBnkRq request;
	@Getter @Setter
	private SSEnt_ChecksListRs response;

	SS_LoggingImpl logr; 
	
	public SSRepo_CheckBookList() {
		response = new SSEnt_ChecksListRs();
		response.setStatus(new SSEnt_Status(getMsg("ST_APPROVED"), getMsg("ER_CLEAN")));
		response.setCheckbooks(null);
		logr = new SS_LoggingImpl();
		logr.setClazz(getClass());
	}
	
	public SSRepo_CheckBookList(SSEnt_HeadInitBnkRq request) {
		this();
	...
	public void readSQL(SQLInput stream, String typeName) {
      ...
      try{
    	  logr.getLog().trace("start readSQL, SSRepo_CheckBookList");
    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
            if(ent_Status.isAuthorized()){
            	logr.getLog().trace("Authorized readSQL, SSRepo_CheckBookList");
            ...
            {
            	logr.getLog().trace("Pas de données disponible list des SSRepo_CheckBookList");
                response.setStatus(new SSEnt_Status(getMsg("ST_REJECTED"), 
                		ent_Status.getErrorCode(), ent_Status.getErrorMsg()));
            }
            logr.getLog().trace("end readSQL, SSRepo_CheckBookList");
      }
      catch(NullPointerException e){
	    	logr.getLog().error("readSQL, SSRepo_CheckBookList",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		logr.getLog().trace("start writeSQL, SSRepo_CheckBookList");
		stream.writeObject(request.getInitiator());
		logr.getLog().trace("end writeSQL, SSRepo_CheckBookList");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_ChecksListRs processing(DataSource dataSource) {
		try {
			logr.getLog().trace("start processing, SSRepo_CheckBookList");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_CheckBookList.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_CheckBookList)jdbcCall.executeObject(SSRepo_CheckBookList.class,in)).getResponse();
			logr.getLog().trace("end processing, SSRepo_CheckBookList");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_CheckBookList",e);

...
public class SSRepo_LinkAccountCard implements SQLData {

	private final String REQUEST_TYPE = "LINKCARDRQ";
	private final String RESPONSE_TYPE = "LINKCARDRS";
	
	private final String PACKAGE_SQL = SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL = "linkAccountCard";

	@Getter @Setter
	private SSEnt_LinkAccountCardRq request;
	...
	public SSRepo_LinkAccountCard(SSEnt_LinkAccountCardRq request) {
		this();
	...
	public void readSQL(SQLInput stream, String typeName) throws SQLException {
      try{
    	  logr.getLog().trace("start readSQL, SSRepo_LinkAccountCard");
    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
            if(!ent_Status.isAuthorized()){
            	logr.getLog().trace("Error readSQL, SSRepo_LinkAccountCard");
                response.setStatus(new SSEnt_Status(getMsg("ST_REJECTED"), 
                		ent_Status.getErrorCode(), ent_Status.getErrorMsg()));
            }
            logr.getLog().trace("end readSQL, SSRepo_LinkAccountCard");
      }
      catch(Exception e){
	    	logr.getLog().error("readSQL, SSRepo_LinkAccountCard",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		logr.getLog().trace("start writeSQL, SSRepo_LinkAccountCard");
		stream.writeObject(request.getInitiator());
		stream.writeString(request.getAccount().getAccountNbr());
		logr.getLog().trace("end writeSQL, SSRepo_LinkAccountCard");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_BaseRs processing(DataSource dataSource) {
		try {
			 logr.getLog().trace("start processing, SSRepo_LinkAccountCard");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_LinkAccountCard.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_LinkAccountCard)jdbcCall.executeObject(SSRepo_LinkAccountCard.class,in)).getResponse();
			 logr.getLog().trace("end processing, SSRepo_LinkAccountCard");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_LinkAccountCard",e);

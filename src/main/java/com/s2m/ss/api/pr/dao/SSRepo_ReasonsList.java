...
public class SSRepo_ReasonsList implements SQLData {

	private final String REQUEST_TYPE 	= "REASONSLISTRQ";
	private final String RESPONSE_TYPE 	= "REASONSLISTRS";
	
	private final String PACKAGE_SQL 	= SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL 	= "getReasonsList";

	@Getter @Setter
	private SSEnt_HeadInitBnkRq request;
	@Getter @Setter
	private SSEnt_ReasonsListRs response;

	SS_LoggingImpl logr; 
	
	public SSRepo_ReasonsList() {
		response = new SSEnt_ReasonsListRs();
	...
	public SSRepo_ReasonsList(SSEnt_HeadInitBnkRq request) {
		this();
	...
	public void readSQL(SQLInput stream, String typeName) {
      ...
      try{
    	  logr.getLog().trace("start readSQL, SSRepo_ReasonsList");
    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
            if(ent_Status.isAuthorized()){
            	logr.getLog().trace("Authorized readSQL, SSRepo_ReasonsList");
        		Object[] objects = (Object[]) stream.readArray().getArray();
        		List<SSEnt_Reason> reasons = Arrays.stream(objects).map(o -> (SSEnt_Reason)o).collect(Collectors.toList());
        		response.setReasons(reasons);
            }
            else
            {
            	logr.getLog().trace("Error readSQL, SSRepo_ReasonsList");
                response.setStatus(new SSEnt_Status(getMsg("ST_REJECTED"), 
                		ent_Status.getErrorCode(), ent_Status.getErrorMsg()));
            }
            logr.getLog().trace("end readSQL, SSRepo_ReasonsList");
      }
      catch(NullPointerException e){
    	  logr.getLog().trace("Pas de données disponible list des SSRepo_ReasonsList");
  			response.setStatus(new SSEnt_Status(getMsg("ST_NODATA"), getMsg("ER_CLEAN")));
      }
      catch(Exception e){
	    	logr.getLog().error("readSQL, SSRepo_ReasonsList",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		logr.getLog().trace("start writeSQL, SSRepo_ReasonsList");
		stream.writeObject(request.getInitiator());
		logr.getLog().trace("end writeSQL, SSRepo_ReasonsList");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_ReasonsListRs processing(DataSource dataSource) {
		try {
			logr.getLog().trace("start processing, SSRepo_ReasonsList");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_ReasonsList.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_ReasonsList)jdbcCall.executeObject(SSRepo_ReasonsList.class,in)).getResponse();
	...
	private Map<String, Class<?>> createMap() {
	    ...
	    myMap.put("REASONSTRUCT", SSEnt_Reason.class);
	    return myMap;
	}
}

...
public class SSRepo_RenewCard implements SQLData {

	private final String REQUEST_TYPE = "REQRENEWRQ";
	private final String RESPONSE_TYPE = "REQRENEWRS";
	
	private final String PACKAGE_SQL = SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL = "reqRenew";
	...
	public SSRepo_RenewCard(SSEnt_HeadInitCrdRq request) {
		this();
	...
	public void readSQL(SQLInput stream, String typeName) throws SQLException {
      try{
    	  logr.getLog().trace("start readSQL, SSRepo_RenewCard");
    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
            if(!ent_Status.isAuthorized()){
            	logr.getLog().trace("Error readSQL, SSRepo_RenewCard");
                response.setStatus(new SSEnt_Status(getMsg("ST_REJECTED"), 
                		ent_Status.getErrorCode(), ent_Status.getErrorMsg()));
            }
            logr.getLog().trace("end readSQL, SSRepo_RenewCard");
      }
      catch(Exception e){
	    	logr.getLog().error("readSQL, SSRepo_RenewCard",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		logr.getLog().trace("start writeSQL, SSRepo_RenewCard");
		stream.writeObject(request.getInitiator());
		logr.getLog().trace("end writeSQL, SSRepo_RenewCard");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_BaseRs processing(DataSource dataSource) {
		try {
			 logr.getLog().trace("start processing, SSRepo_RenewCard");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_RenewCard.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_RenewCard)jdbcCall.executeObject(SSRepo_RenewCard.class,in)).getResponse();
			 logr.getLog().trace("end processing, SSRepo_RenewCard");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_RenewCard",e);

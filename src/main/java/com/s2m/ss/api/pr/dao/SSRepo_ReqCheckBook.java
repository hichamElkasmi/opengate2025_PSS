...
public class SSRepo_ReqCheckBook implements SQLData {

	private final String REQUEST_TYPE = "CHECKBOOKRQ";
	private final String RESPONSE_TYPE = "CHECKBOOKRS";
	
	private final String PACKAGE_SQL = SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL = "reqCheckBook";

	@Getter @Setter
	private SSEnt_CheckbookRq request;
	...
	public SSRepo_ReqCheckBook(SSEnt_CheckbookRq request) {
		this();
	...
	public void readSQL(SQLInput stream, String typeName) throws SQLException {
      try{
    	  logr.getLog().trace("start readSQL, SSRepo_ReplaceCard");
    	    SSEnt_Status ent_Status = (SSEnt_Status)stream.readObject();
            if(!ent_Status.isAuthorized()){
            	logr.getLog().trace("Error readSQL, SSRepo_ReplaceCard");
                response.setStatus(new SSEnt_Status(getMsg("ST_REJECTED"), 
                		ent_Status.getErrorCode(), ent_Status.getErrorMsg()));
            }
            logr.getLog().trace("end readSQL, SSRepo_ReplaceCard");
      }
      catch(Exception e){
	    	logr.getLog().error("readSQL, SSRepo_ReplaceCard",e);
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		logr.getLog().trace("start writeSQL, SSRepo_ReplaceCard");
		stream.writeObject(request.getInitiator());
		stream.writeInt(request.getCheckBook().getCheckCode());
		logr.getLog().trace("end writeSQL, SSRepo_ReplaceCard");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_BaseRs processing(DataSource dataSource) {
		try {
			 logr.getLog().trace("start processing, SSRepo_ReplaceCard");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_ReqCheckBook.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_ReqCheckBook)jdbcCall.executeObject(SSRepo_ReqCheckBook.class,in)).getResponse();
			 logr.getLog().trace("end processing, SSRepo_ReplaceCard");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_ReplaceCard",e);

...
public class SSRepo_Transfer implements SQLData {

	private final String REQUEST_TYPE = "TRANSFERRQ";
	private final String RESPONSE_TYPE = "TRANSFERRS";
	
	private final String PACKAGE_SQL = SSEnt_DF.PACKAGE_SQL;
	private final String PROCEDURE_SQL = "setTransfer";

	
	@Getter @Setter
	private SSEnt_TransfertRq request;
	@Getter @Setter
	private SSEnt_AuthorisationRs response;

	SS_LoggingImpl logr; 
	
	public SSRepo_Transfer() {
	...
	public SSRepo_Transfer(SSEnt_TransfertRq request) {
		this();
	...
	public void writeSQL(SQLOutput stream) throws SQLException {		
		logr.getLog().trace("start writeSQL, SSRepo_Transfer");
		stream.writeObject(request.getInitiator());
		stream.writeObject(request.getTransferTrx());
		logr.getLog().trace("end writeSQL, SSRepo_Transfer");
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SSEnt_AuthorisationRs processing(DataSource dataSource) {
		try {	
			logr.getLog().trace("start processing, SSRepo_Transfer");
	         ...
	         .declareParameters(
		        ...
		        .declareParameters(
		            new SqlOutParameter(
		                ...
		                OracleTypes.STRUCT,
		                RESPONSE_TYPE,
		                new SqlReturnTypeData(SSRepo_Transfer.class, auxiliaryTypes)));
		    
			Map in = Collections.singletonMap("v_request", this);
			response = ((SSRepo_Transfer)jdbcCall.executeObject(SSRepo_Transfer.class,in)).getResponse();
			logr.getLog().trace("end processing, SSRepo_Transfer");
		}catch (Exception e) {  
			logr.getLog().error("processing, SSRepo_Transfer",e);

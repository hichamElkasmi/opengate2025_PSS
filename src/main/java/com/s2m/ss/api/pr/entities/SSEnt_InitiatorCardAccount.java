...
@ToString
public class SSEnt_InitiatorCardAccount extends SSEnt_Initiator implements SQLData {
	...
	@Schema(description = "Numéro du compte, Format RIB")
	...
	public void writeSQL(SQLOutput stream) throws SQLException {
		...
		log.getLog().trace("start writeSQL SSEnt_InitiatorCardAccount");

		stream.writeString(String.valueOf(requestId));
		stream.writeString(clientId);
		stream.writeDouble(cardCode);
		stream.writeString(accountNbr);
		
		log.getLog().debug(this.toString());
		log.getLog().trace("end writeSQL SSEnt_InitiatorCardAccount");
	}
}
